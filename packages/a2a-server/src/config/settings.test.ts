...
describe('loadSettings', () => {
  const mockHomeDir = path.join(os.tmpdir(), 'gemini-a2a-test-home');
  const mockWorkspaceDir = path.join(os.tmpdir(), 'gemini-a2a-test-workspace');
  ...
  beforeEach(() => {
    ...
    // Clean up settings files before each test if they exist
